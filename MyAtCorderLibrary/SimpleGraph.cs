...
namespace MyAtCoderLibrary.Graph
{
        ...
        , System.Numerics.IMinMaxValue<T>
    {
        ...
        public SimpleGraph(bool isUnDirected, int vertexCount)
        {
            ...
            this.visited = new bool[vertexCount];
        }
        ...
        public void AddEdge(int from, int to, T length)
        {
            ...
            // fromとtoが同じか、すでに頂点が辺を含んでいるか、辺の長さがゼロ未満なら何もせずに戻る。
        ...
        public (T result, int[] path) SearchPathByFunc(int s, int t, T init, Func<T, T, bool> judgeFunc, OPERATOR op = OPERATOR.加算)
        {
            // 1から始まる頂点指定を0始点に変換。
            s--;
            t--;
            ...
            // 全パスを取得する。
            Span<int[]> paths = FindPaths(s, t, new Stack<int>(), new List<int[]>());

            // 重み計算用のFuncを取得。
            Func<T, T, T> calc = (Func<T, T, T>)MathExtension.GetCalculator<T>(op);
        ...
        private Span<T> BFS(int s)
        {
            ...
            // 距離配列を返す。
            return distance.ToArray().AsSpan();

        }

        /// <summary>
        /// 全頂点への最短距離を求めるためのダイクストラ法。<br/>
        /// 軽く原理を記録しておくと、ある頂点から到達可能な頂点を次々に優先度付きキューに追加して、近い順に頂点が取り出されるようにするというところ。
        /// スタート地点からグラフ内の全頂点への最短距離を記録したSpanを返すオーバーロード。<br/>
        /// 目的地t に関する条件分岐を消せるので、同じコードの繰り返しにはなるがここで再定義する。
        /// </summary>
        ...
        private Span<T> DkStr(int s)
        {
            ...
            // BFSは各頂点を一度しか訪問しないので訪問済みの頂点番号は記録する
            visited.AsSpan().Fill(false);
        ...
        /// <returns></returns>
        private Span<int[]> FindPaths(int current, int target, Stack<int> path, List<int[]> paths, bool isFirst = true)
        {
            ...
            else
            {
                ...
                for ( int i = 0; i < edgeSpan.Length; i++ )
                {
                    ...
                    {
                        FindPaths(edgeSpan[i].Item1, target, path, paths, false);
                    }
                }
            }
